...
function Member.object:set_password(password)
    ...
    local function rounds()
        return multirand.integer(config.password_hash_min_rounds,
    ...
    hash_prefix = hash_prefix .. multirand.string(salt_length,
...
function Member.object_get:published_contacts()
    return Member:new_selector():join('"contact"', nil, '"contact"."other_member_id" = "member"."id"'):add_where { '"contact"."member_id" = ?', self.id }:add_where("public"):exec()
end
...
function Member:get_search_selector(search_string)
    return self:new_selector():add_field({ '"highlight"("member"."name", ?)', search_string }, "name_highlighted"):add_where { '"member"."text_search_data" @@ "text_search_query"(?)', search_string }:add_where("activated NOTNULL AND active")
end
...
function Member.object:send_invitation(template_file, subject)
  ...
  local parelonBaseUrl = "https://peoschool.alumbria.it/"
  ...
  if template_file then
    ...
    content = slot.use_temporary(function()
      slot.put(_"Hello\n\n")
  		       ...
  		       slot.put("https://peoschool.alumbria.it/index/register.html?invite=" .. self.invite_code .. "\n\nbefore 24h.")
  		       slot.put(_"If this link is not working, please open following url in your web browser:\n\n")
  		       slot.put("https://peoschool.alumbria.it/index/register.html\n\n")
...
function Member.object:set_notify_email(notify_email)
    ...
    local content = slot.use_temporary(function()
        ...
        slot.put("https://peoschool.alumbria.it/index/confirm_notify_email.html?secret=" .. self.notify_email_secret .. "\n\n")
        slot.put(_ "If this link is not working, please open following url in your web browser:\n\n")
        slot.put("https://peoschool.alumbria.it/index/confirm_notify_email.html\n\n")
...
function Member.object:get_setting_maps_by_key(key)
    return SettingMap:new_selector():add_where { "member_id = ?", self.id }:add_where { "key = ?", key }:add_order_by("subkey"):exec()
end

function Member.object:get_setting_map_by_key_and_subkey(key, subkey)
    return SettingMap:new_selector():add_where { "member_id = ?", self.id }:add_where { "key = ?", key }:add_where { "subkey = ?", subkey }:add_order_by("subkey"):optional_object_mode():exec()
end
...
function Member.object_get:notify_email_locked()
    return (Member:new_selector():add_where { "id = ?", app.session.member.id }:add_where("notify_email_lock_expiry > now()"):count() == 1)
end

function Member.object_get:units_with_voting_right()
    return (Unit:new_selector():join("privilege", nil, { "privilege.unit_id = unit.id AND privilege.member_id = ? AND privilege.voting_right", self.id }):exec())
end
...
function Member.object:ui_field_text(args)
    ...
    if app.session:has_access("authors_pseudonymous") then
        ...
        ui.container {
            label = args.label,
            label_attr = { class = "ui_field_label" },
            content = function()
...
function Member.object:has_voting_right_for_unit_id(unit_id)
    if not self.__units_with_voting_right_hash then
        local privileges = Privilege:new_selector():add_where { "member_id = ?", self.id }:add_where("voting_right"):exec()
...
function Member.object:has_polling_right_for_unit_id(unit_id)
    if not self.__units_with_polling_right_hash then
        local privileges = Privilege:new_selector():add_where { "member_id = ?", self.id }:add_where("polling_right"):exec()
